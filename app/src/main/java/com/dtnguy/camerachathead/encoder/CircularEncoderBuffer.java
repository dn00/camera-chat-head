...
public class CircularEncoderBuffer {
    ...
    public long getPTSatNSec(int n) {
        ...
        //long s =  mPacketPtsUs[index] - (1000000L * n);
        //Log.d(TAG, "getPTSatNSec " + s);
        return  mPacketPtsUs[index] - (1000000L * n);

    }

    public int getIndexAtPTSVideo(long s) {
        final int metaLen = mPacketStart.length;
        int index = (mMetaHead + metaLen - 1) % metaLen;
        ...
        do
        {
            ...
            if (index == 0) {
                if (mPacketPtsUs[(index + metaLen - 1) % metaLen] == 0) {
                    return 0;
                }
            }
            if ((mPacketFlags[index] & MediaCodec.BUFFER_FLAG_KEY_FRAME) != 0) {
                prevKeyFrameIndex = index;
            }
...
//        } while ((mPacketFlags[index] & MediaCodec.BUFFER_FLAG_KEY_FRAME) == 0);
        ...
        return prevKeyFrameIndex;
    }

    public int getIndexAtPTSAudio(long s) {

        final int metaLen = mPacketStart.length;
        int index = (mMetaHead + metaLen - 1) % metaLen;
        //long test = (mPacketPtsUs[index]) - (s + (n*1000000L));
...
//        }
    ...
    public int getPrevIndexVideo(int index) {
        final int metaLen = mPacketStart.length;
        int next = (index - 1) % metaLen;
        if (next == mMetaHead) {
            return -1;
        ...
        return (mMetaHead + mPacketStart.length - n) % mPacketStart.length;
    }
