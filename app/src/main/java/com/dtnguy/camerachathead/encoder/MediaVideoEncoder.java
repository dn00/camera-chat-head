...
import com.dtnguy.camerachathead.glutils.RenderHandler;
...
public class MediaVideoEncoder extends MediaEncoder {
    ...
    public static final int circBufferUs = 25000;
    ...
    public boolean frameAvailableSoon(final float[] tex_matrix, final float[] mvp_matrix, Bitmap bm) {
        boolean result;
        if (result = super.frameAvailableSoon())
            mRenderHandler.draw(tex_matrix, mvp_matrix);

        if (doWeNeedAFrame()) {
            super.requestFrameListUpdate(bm);
            doWeNeedFrame = false;
        }
        return result;
    }

    public void addAFrame(Bitmap bm) {
        super.requestFrameListUpdate(bm);
        doWeNeedFrame = false;

    }
    public void setDoWeNeedFrameFalse() {
        doWeNeedFrame = false;
    }
    public boolean doWeNeedAFrame() {
        return doWeNeedFrame;
    }
    ...
    protected void prepare() throws IOException {
        ...
        mMediaCodec.start();
        mEncBuffer = new CircularEncoderBuffer(format, circBufferUs);
